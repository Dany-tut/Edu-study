...
/**
 * Порог перекоса — в СИГМАХ биномиального разброса, а не в процентных пунктах.
 *
 * ПОЧЕМУ НЕ ПУНКТЫ. Пункт весит тем больше, чем меньше набор: в `ruvo` 243
 * наблюдения, и одно место гуляет там на три пункта само по себе. Плоский
 * потолок «6 п.п.» на таком курсе срабатывает уже на 1.9σ — то есть ловит
 * обычный жребий, — а на курсе в две тысячи вопросов пропускает настоящую
 * привычку автора. Сигма приводит оба случая к одной мерке: сколько ответов
 * село бы на это место само по себе, ± сколько такой счёт гуляет.
 *
 * 2.5σ выбраны по данным диагностик: до правки корейский placement давал 8.6σ,
 * английский 6.2σ, химия 3.3σ — настоящая привычка уходит далеко за порог, а
 * разброс на банке в двадцать вопросов до него не достаёт. На курсах запас тот
 * же: перекос второго места, который был до Р15 (3.7 п.п. по всем курсам,
 * 17.8 в `ielt`), это 10.6σ и 9.8σ — ловится с многократным запасом.
 */
const SIGMAS = 2.5
/** На сколько сигм место перебирает свой ожидаемый счёт. */
const sigmaOver = (acc, place) => {
  const expect = acc.chance                       // сумма 1/N по вопросам набора
...
].filter(([, acc]) => acc.n >= 150 && worstSigma(acc)[1] > SIGMAS)
if (bad.length) {
  console.log('')
  for (const [where, acc] of bad) {
    const [p, sg] = worstSigma(acc)
    console.log(`  ✗ ${where}: место ${p + 1} перебирает случай на ${sg.toFixed(1)}σ (потолок ${SIGMAS}σ), это ${at(acc, p).toFixed(1)} п.п. на ${acc.n} ответах`)
  }
  console.log('\nМесто верного ответа раскладывает editorTask (src/data/languageCourse.ts) через placeCorrect —')
  console.log('солью служит id задания, то есть урок плюс номер в уроке (Р15). Задание, которое не проходит')
  console.log('через editorTask, туда не попадает — зовите placeCorrect/answerSide/spreadMulti вручную.')
  process.exit(1)
}
...
console.log(`\n✓ ни одно место не перебирает случай больше чем на ${SIGMAS}σ — вслепую не пройти`)
