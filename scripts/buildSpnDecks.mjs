  ...
  for (const f of readdirSync(dir).filter(f => f.endsWith('.ts') && !SKIP.has(f))) {
    ...
    for (const m of src.matchAll(/(?:term: |c\(\s*)'((?:[^'\\]|\\.)*)'/g)) {
      for (const w of m[1].replace(/\\'/g, "'").toLowerCase().split(/[^a-z'’]+/)) {
        ...
        // И ходовые формы того же слова. Лемматизация тут не спасает: она
        // приводит форму к основе ТОЛЬКО если основы нет в частотном списке, а
        // «cremated», «hunted», «dreamt» стоят в нём отдельными строками и
        // остаются собой. В итоге «to cremate» из готового набора не сходится с
        // «cremated» из субтитров, и разобранное слово едет в колоду второй раз.
        const stem = norm.replace(/e$/, '')
        for (const f of [`${norm}s`, `${norm}d`, `${norm}ed`, `${stem}ed`, `${stem}ing`, `${norm}ing`]) seen.add(f)
      }
    }
  }
  return seen
