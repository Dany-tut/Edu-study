...
// Два варианта об одном смысле — уточнение в скобках у того же слова. Живая
// колода давала «спасибо» и «спасибо (мягче)» в одном задании: формально
// четыре варианта, на деле три.
const DUPES = [
  card('d1', '매운', 'острый'),
  card('d2', '감사합니다', 'спасибо'),
  card('d3', '고마워요', 'спасибо (мягче)'),
  card('d4', '없어요', 'нет (не имеется)'),
  card('d5', '아니요', 'нет'),
  card('d6', '늦게', 'поздно'),
  card('d7', '버스', 'автобус'),
  card('d8', '사람', 'человек'),
]
const dupes = buildReviewTasks([DUPES[0]], DUPES)[0]
const pairs = dupes.kind === 'choice'
  ? dupes.choices.filter((c, i) => dupes.choices.some((o, j) => j !== i && (o + ' ').startsWith(c + ' ')))
  : []
if (pairs.length === 0) ok(`два варианта об одном смысле не стоят рядом (${dupes.choices.join(' / ')})`)
else fail('в задании два варианта об одном смысле', pairs.join(' / '))

// Породы ответов. Колода одна на предмет, а карточки в ней трёх жанров, и
// ответ каждого — правильный ответ на СВОЙ вопрос. Подставленный к чужому, он
// отсекается не глядя.
const KINDS = [
  card('k1', '매운', 'острый'),
  card('k2', 'Как звучит 옷을?', '[오슬] — одежду (винительный)', { source: 'trainer' }),
  card('k3', 'Как звучит 한국어?', '[한구거] — корейский язык', { source: 'trainer' }),
  card('k4', 'Что происходит с 받침 перед гласной?', 'Он переходит в начало следующего слога', { source: 'homework' }),
  card('k5', 'ㄹ', 'р'),
  card('k6', '재미있어요', 'интересно'),
  card('k7', '맛있어요', 'вкусно'),
  card('k8', '늦게', 'поздно'),
]
const spicy = buildReviewTasks([KINDS[0]], KINDS)[0]
if (spicy.kind === 'choice' && !spicy.choices.some(c => c.startsWith('[')))
  ok(`чтение в скобках не идёт обманкой к значению (${spicy.choices.join(' / ')})`)
else fail('в варианты к значению попало чтение', JSON.stringify(spicy.choices))
if (spicy.kind === 'choice' && !spicy.choices.some(c => c.split(/\s+/).length >= 4))
  ok('формулировка правила не идёт обманкой к значению')
else fail('в варианты к значению попала формулировка правила', JSON.stringify(spicy.choices))

// А своему вопросу чтение остаётся верным ответом — правило про смешивание, а
// не про запрет таких карточек.
const sound = buildReviewTasks([KINDS[1]], KINDS)[0]
if (sound.kind === 'choice' && sound.choices[sound.correct] === '[오슬]')
  ok('вопрос «как звучит» по-прежнему отвечается чтением')
else fail('карточка чтения потеряла свой верный ответ', JSON.stringify(sound))
...
const MIXED = [
  ...
  card('m11', '수고하셨습니다', 'спасибо за работу'),
  card('m12', '괜찮아요', 'ничего страшного'),
]
