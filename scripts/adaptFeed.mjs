...
// ─── Дыры разбора ────────────────────────────────────────────────────────────
//
// Ровно то же определение, что у сторожа (scripts/checkGloss.mjs): дыра — когда
// тап не даёт НИ перевода, НИ разбора по составу. Слово, у которого есть
// состав, тупиком не является.
function holes(lang, text, extra) {
  const lex = M.buildLexicon(lang, extra)
  const out = new Set()
  for (const seg of lex.segment(text)) {
    if (seg.word && !seg.gloss && !seg.parts?.some(p => p.gloss)) out.add(seg.text)
  }
  return [...out]
}
