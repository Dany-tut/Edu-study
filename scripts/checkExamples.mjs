...
function contains(sentence, term, key, lang) {
  ...
  // Дефис — только в НАЧАЛЕ: внутри слова он часть самого слова
  // («B1-Zertifikat»), а в начале это корейская грамматическая модель
  // («-라고 합니다», «-어 놓다») — искать её в предложении так же нечего.
...
//
// 26.08.2026 долг закрыт целиком: последние 102 слова (58 немецких, 21
// корейское, 13 японских, 10 английских) получили примеры руками. Нули здесь
// теперь значат простое: слово без примера — ошибка, а не наследство.
// Возвращать сюда число — только вместе с объяснением, почему пример написать
// нельзя.
const DEBT = { en: 0, ja: 0, ko: 0, pt: 0, de: 0 }
