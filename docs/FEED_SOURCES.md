...
## Сырьё для пересказов (`facts: true`) — работает автоматически
...
текст — этим занимается `scripts/adaptFeed.mjs`. `buildFeed` складывает их в
`scripts/feed-staging/facts.json`, оттуда конвейер и берёт.

**Язык источника значения не имеет.** Заметка N+1 по-русски годится в сырьё для
корейского пересказа ровно так же, как релиз NASA по-английски: пересказ на
другом языке копией чужого изложения быть не может по построению.

Все проверены запросом 26.08.2026, все отдают описание, а не один заголовок.

### Русскоязычные

| Источник | Тема | Адрес |
|---|---|---|
| N+1 | наука, биология, археология, медицина | `https://nplus1.ru/rss` |
| Хабр | техника, ИТ | `https://habr.com/ru/rss/news/?fl=ru` |
| Archi.ru | архитектура | `https://archi.ru/rss.xml` |
| Euronews | мир, технологии | `https://ru.euronews.com/rss` |

### Корейские издания

Для ленты закрыты, для пересказа — лучший источник именно корейской повестки:
что в Корее считают новостью, а не что о Корее пишут снаружи.

| Источник | Тема | Адрес |
|---|---|---|
| BBC News 코리아 | мир | `https://feeds.bbci.co.uk/korean/rss.xml` |
| 연합뉴스 | Корея и мир | `https://www.yna.co.kr/rss/news.xml` |
| 전자신문 | электроника, ИИ, полупроводники | `https://rss.etnews.com/Section901.xml` |
| ZDNet Korea | ИТ, гаджеты | `https://feeds.feedburner.com/zdkorea` |
| AI타임스 | ИИ целиком | `https://www.aitimes.com/rss/allArticle.xml` |
| 월간 디자인 | дизайн | `https://design.co.kr/rss` |
| 메디컬투데이 | медицина | `https://www.mdtoday.co.kr/rss/allArticle.xml` |
| 한겨레 | общество | `https://www.hani.co.kr/rss/` — настроен, но **стоп-список съедает почти всё**: повестка политическая |

### Биология из открытых журналов

Копировать их аннотации в ленту нельзя **по языку, а не по праву**: они
написаны для коллег (C1–C2). Для пересказа годятся идеально, и CC BY разрешает
переработку тоже.
...
| 동아사이언스, 사이언스타임즈, YTN 사이언스 | фида нет ни по одному известному адресу, сайты на JS |
| SPACE (vmspace.com) | то же — фид не отвечает |
| neurocampus.su | сайт на Tilda, весь контент рисуется скриптом: ни фида, ни ссылок в разметке |
