  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12.5, color: 'var(--color-text-3)' }}>
        <span>{t('Использовано')}: <b style={{ color: 'var(--color-text)' }}>{fmtBytes(used)}</b></span>
        <span>{t('Лимит')}: <b style={{ color: 'var(--color-text)' }}>{fmtBytes(limit)}</b></span>
        <span style={{ color: c.text, fontWeight: 700 }}>{t('Свободно')}: {fmtBytes(limit - used)}</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', scrollbarGutter: 'stable' }}><div style={{ maxWidth: 760, margin: '0 auto', padding: '28px 24px 60px', display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div>
          <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--color-text)', letterSpacing: '-0.3px' }}>{t('Хранилище')}</div>
          <div style={{ fontSize: 12, color: 'var(--color-text-3)', marginTop: 1 }}>{t('База данных и файлы')}</div>
        </div>
        <motion.button
          whileTap={{ scale: 0.94 }}
          onClick={load}
      ...
      {error ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--color-text-3)', fontSize: 13, padding: '24px 0' }}>
          <AlertTriangle size={16} /> {t('Не удалось загрузить статистику')}
        </div>
      ) : (
        <>
          {stats && <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <BigUsageBar icon={Database} label={t('База данных (Postgres)')} used={stats.db_bytes} limit={stats.db_limit_bytes} />
            <BigUsageBar icon={HardDrive} label={t('Файлы (Storage)')} used={stats.storage_bytes} limit={stats.storage_limit_bytes} hint={`${stats.storage_objects} ${t('объект.')}`} />
          </div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 12.5, color: 'var(--color-text-2)', lineHeight: 1.6 }}>
              {t('Фото и рисунки доски хранятся в строках ДЗ внутри базы данных (не в Storage):')}&nbsp;
              <b style={{ color: 'var(--color-text)' }}>{fmtBytes(stats.attachments_bytes)}</b> {t('в')} {stats.attachments_rows} {t('записях.')}
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[...stats.tables].sort((a, b) => b.bytes - a.bytes).map(tbl => (
                <div key={tbl.name} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <span style={{ fontSize: 12.5, color: 'var(--color-text-2)', width: 160, flexShrink: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{tbl.name}</span>
                  <div style={{ flex: 1, height: 8, borderRadius: 5, background: 'var(--color-bg-4)', overflow: 'hidden' }}>
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${(tbl.bytes / maxTableBytes) * 100}%` }}
                      transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
                      style={{ height: '100%', borderRadius: 5, background: 'var(--color-accent)', opacity: 0.75 }}
                    />
                  </div>
                  <span style={{ fontSize: 12, color: 'var(--color-text-3)', width: 70, textAlign: 'right', flexShrink: 0 }}>{fmtBytes(tbl.bytes)}</span>
                  <span style={{ fontSize: 11, color: 'var(--color-text-4)', width: 60, textAlign: 'right', flexShrink: 0 }}>{tbl.rows} {t('стр.')}</span>
                </div>
              ))}
            </div>
          </div>
