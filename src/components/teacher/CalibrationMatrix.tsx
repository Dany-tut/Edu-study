...
export default function CalibrationMatrix({ owner, subject }: {
  ...
  if (!c) return null
  if (c.total === 0) return (
    <div style={{ fontSize: 12, color: 'var(--color-muted)', padding: '8px 0' }}>Калибровка уверенности не собрана (тест без опроса уверенности).</div>
  )

  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 10 }}>
        <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--color-text)' }}>Калибровка уверенности</span>
        <span style={{ fontSize: 12, color: 'var(--color-muted)' }}>точность самооценки: <b style={{ color: c.calibration >= 60 ? 'var(--color-green-text)' : 'var(--color-red-text)' }}>{c.calibration}%</b></span>
      </div>
          ...
          return (
            <div key={q.key} style={{ padding: '10px 12px', borderRadius: 12, border: `1.5px solid ${q.color}55`, background: `${q.color}12` }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--color-text)' }}>{q.label}</span>
                <span style={{ fontSize: 13, fontWeight: 800, color: q.color }}>{n}</span>
              </div>
              <div style={{ fontSize: 10.5, color: 'var(--color-text-3)', marginTop: 2 }}>{q.hint} · {pct}%</div>
            </div>
          )
        })}
      </div>
      {c.confidentWrong > 0 && (
        <div style={{ marginTop: 10, fontSize: 12, color: 'var(--color-red-text)', lineHeight: 1.5 }}>
          ⚠ {c.confidentWrong} ответ(ов) «уверен, но неверно» — стоит разобрать: ученик уверен в неправильном.
        </div>
      )}
    </div>
  )
