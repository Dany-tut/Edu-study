  ...
  return (
    <motion.div
      ...
      {!compact && task.questionTable && (
        <div style={{ borderRadius: 12, overflow: 'hidden', border: '1px solid var(--color-border-medium)', alignSelf: 'flex-start', minWidth: '50%', maxWidth: '100%' }}>
          <table style={{ borderCollapse: 'collapse', fontSize: 13, width: '100%' }}>
            ...
            <tbody>{task.questionTable.rows.map((row, ri) => (
              <tr key={ri}>{row.map((cell, ci) => (
                <td key={ci} style={{ borderTop: ri > 0 ? '1px solid var(--color-border)' : undefined, borderRight: '1px solid var(--color-border)', padding: '8px 14px', background: 'var(--color-table-cell-bg)' }}>{cell}</td>
              ))}</tr>
            ))}</tbody>
          </table>
        </div>
...
function BankGridCard({
  ...
  return (
    ...
    >
      ...
      {/* Question preview */}
      <p style={{ margin: 0, fontSize: 12.5, fontWeight: 600, lineHeight: 1.4, color: 'var(--color-text)', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
        {task.question.split('\n')[0] || <span style={{ color: 'var(--color-text-4)' }}>Без текста</span>}
      </p>

      {/* Topic */}
      <span style={{ fontSize: 10.5, color: 'var(--color-text-3)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginTop: 'auto' }}>{task.topic}</span>
