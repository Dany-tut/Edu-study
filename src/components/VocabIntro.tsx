 ...
 *
 * ГОТОВЫХ СЛОГОВ ХАНГЫЛЯ (U+AC00–U+D7AF) ЗДЕСЬ НЕТ, И ЭТО ГЛАВНОЕ.
 * Сначала они в списке были — по виду это тоже «один символ». Но слог хангыля
 * не буква: 손 «рука», 물 «вода», 책 «книга», 방 «комната» — полноценные слова,
 * а односложных слов в корейском половина словаря. В юните, где рядом стоят 손
 * и 학교, первое печаталось в 40 пунктов по центру, второе — в 18 по левому
 * краю: ряд разъезжался, и разной оказывалась не природа карточки, а длина
 * слова.
 *
 * Буквы алфавита попадают сюда и без этого диапазона. Хангыль записывает их
 * не готовыми слогами, а совместимыми чамо (ㄱ — U+3131, ㅏ — U+314F), то есть
 * блоком U+3130–U+318F, который в списке остался; ровно ими и заполнен словарь
 * курса алфавита (см. koreanHangul.ts, unitVocab). Курс алфавита не теряет
 * ничего, а курс лексики перестаёт кричать на каждом втором слове.
 */
const GLYPH_RE = /^[\u1100-\u11FF\u3040-\u30FF\u3130-\u318F\u3400-\u9FFF]$/
                ...
                {(walking && current ? [current] : words).map(w => {
                  ...
                  <button
                    ...
                    style={{
                      ...
                      // Содержимое прижато к низу. Сетка тянет все карточки ряда
                      // до высоты самой высокой, и карточка без рисунка начинала
                      // писать слово там, где у соседки с рисунком ещё картинка:
                      // строки в ряду стояли лесенкой. Прижатое к низу
                      // содержимое даёт одну линию у всех — с рисунком и без,
                      // с одной строкой перевода и с двумя. Знак — по центру:
                      // он тут единственное содержимое карточки.
                      justifyContent: glyph ? 'center' : 'flex-end',
