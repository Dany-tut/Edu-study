...
import { ChevronDown, ChevronRight, ChevronLeft, Search, Check, SlidersHorizontal, Filter, Layers, Link2, Share2, HelpCircle } from 'lucide-react'
 ...
 * статусов на 0.62 — над плывущими карточками они читались по-разному, будто
 * из разных материалов. Плотность высокая нарочно: на просвет одинаковая
...
export function Tile({ children, onClick, accent, stack, tint }: {
   ...
   *
   * Отличается подложкой в цвете предмета, а не размером или местом: в сетке
   * из сорока одинаковых карточек цвет — единственная метка, которую видно,
   * не читая. `surface` — заливка (палитра предмета, `soft`). Рамки в покое
   * нет и у неё: одна обведённая плитка среди безрамочных читается как
   * выделение, а не как метка. `border` красит только листы стопки, где
   * заливка на просвет слишком слаба, чтобы очертить лист.
   */
