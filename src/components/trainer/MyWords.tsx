...
export function MyWordsTile({ words, states, ready, accent, soft, onOpen }: {
  ...
  return (
    ...
    // синий весь корейский. Заливка — не ровный слой, а свет из нижнего правого
    // угла (см. tintWash): к верхнему левому он сходит в ноль, поэтому чипс и
    // заголовок стоят на чистой подложке, а у угла цвету можно дать 22% —
    // ровная заливка такой силы спорила бы с текстом на себе.
    <Tile accent={accent} stack tint={{ surface: `${accent}38`, border: `${accent}4d` }} onClick={onOpen}>
