    ...
    >
      ...
      >
        ...
        {/* «Верно» полоса не повторяет: этот вердикт напечатан на карточке, у
            ...
            Голое «Не совсем» — тоже повтор: на карточке уже стоит «Неверно» и
            подсвечен верный вариант. На телефоне такая плашка занимает строку
            над кнопкой, ничего не добавляя, поэтому там её нет — остаётся
            только разбор с текстом правильного ответа. */}
        {verdict !== 'none' && verdict !== 'correct' && !(isMobile && verdict === 'wrong' && !answer) && (
          <motion.div
