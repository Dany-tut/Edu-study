...
//
// Нужен свайпу назад (lib/useSwipeBack.ts): чтобы страница уезжала из-под
// пальца, а под ней лежал предыдущий экран, нужны ДВА неподвижных слоя —
// уходящий и открывающийся. Живой React-экран для этого не годится: под
// пальцем он должен замереть, а предыдущего в дереве уже нет вовсе (при
// переходе старый экран размонтируется).
//
...
export type Snapshot = {
  ...
  scrollY: number
  /** Когда сняли (мс). Протухшие кандидаты не показываем. */
  at: number
}
...
export function captureScreen(): Snapshot | null {
  ...
  return { layers, scrollY: Math.round(window.scrollY), at: Date.now() }
}
