...
function drawEmblem(
  ...
  if (emblem === 'rocket') {
    ...
    // Кристалл нельзя центровать по габариту, как звезду или молнию: широкая
    // корона сверху и остриё снизу, поэтому чернила собраны в верхней половине.
    // Замер по альфа-каналу: у звезды/молнии/ракеты центр массы лежит на нуле
    // (±0.006), у симметричного по габариту камня — на -0.20, и он читался как
    // задранный. Габарит опущен до -0.66…1.02, центр массы вернулся на ноль.
    ctx.beginPath()
    ctx.moveTo(-0.69, -0.66)
    ctx.lineTo(0.69, -0.66)
    ctx.lineTo(1.1, -0.13)
    ctx.lineTo(0, 1.02)
    ctx.lineTo(-1.1, -0.13)
    ctx.closePath()
    ...
    ctx.lineWidth = 0.1
    ctx.beginPath()
    ctx.moveTo(-1.1, -0.13); ctx.lineTo(1.1, -0.13)
    ctx.moveTo(-0.69, -0.66); ctx.lineTo(-0.39, -0.13); ctx.lineTo(0, 1.02)
    ctx.moveTo(0.69, -0.66); ctx.lineTo(0.39, -0.13)
    ctx.stroke()
    ctx.strokeStyle = tier.inkDark
