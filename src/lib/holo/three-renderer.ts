/*
 * Портировано из holosticker (https://github.com/jal-co/holosticker) — MIT.
 * MIT License · Copyright (c) 2026 Justin Levine.
 *
 * WebGL-рендер голографического стикера: дифракционные полосы, тонкоплёночная
 * иридесценция, металлические блёстки, студийное окружение, die-cut рамка
 * через euclidean distance transform и 3D-отклейка уголка.
 *
 * Файл — почти дословная копия апстрима (правок нет, кроме этой шапки), чтобы
 * можно было подтягивать обновления. Наши пресеты — в ./presets.ts,
 * React-обёртка — в src/components/HoloSticker.tsx.
 */
...
export class HoloRenderer {
  ...
  /* Экспорт GIF/видео/GLB из апстрима вырезан (маркер react-export:strip):
     тянет gifenc и GLTFExporter, в кабинете не нужен. */


  /**
   * Наша добавка к апстриму: React размонтирует стикеры пачками, а число
   * WebGL-контекстов в браузере ограничено (~16) — освобождаем явно.
   */
  dispose() {
    this.geometry.dispose()
    this.material.uniforms.uMap.value?.dispose?.()
    this.material.uniforms.uHeight.value?.dispose?.()
    this.material.uniforms.uEnv.value?.dispose?.()
    ...
    ;(this.shadowMat as THREE.MeshBasicMaterial).map?.dispose()
