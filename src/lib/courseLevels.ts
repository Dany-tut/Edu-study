...
// общий список не годится: у корейского нет ЕГЭ, у химии нет A2, а у самого
// корейского нет и A2 — он меряется TOPIK. Поэтому набор зависит от предмета.

/**
 * Школьный набор — предметы без языковой шкалы.
 *
 * AP и «Углублённый» здесь не для красоты: курсы с такими уровнями у нас уже
 * есть (AP Chemistry), и пока их не было в справочнике, формы создания
 * предлагали свои списки в обход него — уровень, выбранный в одной форме, во
 * второй не предлагался вовсе.
 */
const SCHOOL_LEVELS = ['ЕГЭ', 'ОГЭ', 'AP', 'Олимпиада', 'Углублённый', 'Школа', 'Интенсив']
...
export function levelOptionsForSubject(subject: string | undefined | null): string[] {
  ...
  const native = NATIVE_SCALE[def.id]
  return [
    // У языка со своей шкалой CEFR НЕ предлагается. Официальных A1–C2 у
    // корейского и японского нет: «A2» там — вольный пересказ ступени, а не
    // уровень, который можно предъявить. Фильтр это и так знает (родная шкала
    // вытесняет CEFR из корзин, см. levelBuckets), и пока чипы A1–C2 висели в
    // выборе, учитель мог поставить курсу уровень, которым этот язык не
    // меряется. Уже проставленный вручную уровень при этом не пропадает:
    // редактор дописывает его отдельным чипом.
    ...(native ?? CEFR),
    ...(LANG_EXAMS[def.id] ?? []),
    'Интенсив',
  ]
