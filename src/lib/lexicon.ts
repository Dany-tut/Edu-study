...
interface KoTail {
   ...
   * связка основу не меняют). 'hada' и 'doeda' — существительное с 하다/되다.
   */
  kind: 'verb' | 'noun' | 'hada' | 'doeda'
  note: string
}
...
const KO_TAILS: KoTail[] = [
  ...
  ...[
    '이었습니다', '였습니다', '이라고', '이라는', '이에요', '이었다', '입니다',
    '예요', '였다', '이다', '이며', '이고', '인', '임',
  ...
  const koDerived = (word: string): WordGloss | undefined => {
    for (const { tail, kind, note } of KO_TAILS) {
      ...
      for (const form of forms) {
        ...
        if (KO_PARTICLES.has(tail) && /частиц/i.test(g.note ?? '')) continue
    ...
    // ПОСЛЕДНЯЯ ПОПЫТКА — для окончания, которое целиком уместилось БУКВОЙ
    // внутри последнего слога: 만든 ← 만들다, 산 ← 살다, 잡은 ← 잡다. Отрезать
    // тут нечего, поэтому пробуем сам слог, разобранный на буквы.
    //
    // Правило намеренно стоит последним и включается, только если финальная
