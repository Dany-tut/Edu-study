...
function buildStage(under: Snapshot | null): Stage {
  ...
  const pairMorphs = (from: HTMLElement, to: HTMLElement | null) => {
    ...
    for (let i = 0; i < pairs.length; i++) {
      ...
      // ── Ход СВОЕЙ таблетки ──
      //
      // Не общий ход страницы, а стык. Отсчёт — от ПРАВОГО края таблетки:
      // морф начинается, когда карточка ВЫШЛА из-под неё целиком, а не когда
      // только коснулась края. Иначе стрелка гасла ещё стоя на карточке —
      // человек видит, что до кнопки свайп даже не дошёл, а её уже нет.
      //
