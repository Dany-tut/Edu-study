...
function buildStage(under: Snapshot | null): Stage {
  ...
  const pairMorphs = (fromChipsAll: HTMLElement[], toChipsAll: HTMLElement[]) => {
    ...
    for (const { el: twin, box, rb } of loners) {
      ...
      hide(twin)
      const inner = wrapKids(clone)
      // ── ПРОЯВЛЯЕТСЯ ПОЗАДИ СТЫКА, А НЕ ПЕРЕД НИМ ──
      //
      // Уходящая таблетка начинает гаснуть, едва стык её надкусил (SEAM_BITE):
      // она уже отдана нижнему экрану. У приходящей ход зеркальный по смыслу, а
      // не по формуле: она принадлежит нижнему экрану и не имеет права
      // показываться там, где ещё лежит верхний. Пока начало считалось той же
      // четвертью, широкая таблетка курса (её левый край у самого поля шапки)
      // проявлялась почти с первого пикселя жеста и висела размытым пятном
      // поверх ещё не уехавшего урока.
      //
      // Поэтому ход КОНЧАЕТСЯ на правом крае таблетки: к моменту, когда стык её
      // прошёл, она уже вся на месте, а до того её закрывает верхний экран.
      const done = rb.left + rb.w
      const span = Math.min(SEAM_SPAN, Math.max(8, done))
      const start = done - span
      const fade = (raw: number) => smooth(Math.min(1, Math.max(0, (raw * W - start) / span)))
      morphs.push({
        el: clone,
