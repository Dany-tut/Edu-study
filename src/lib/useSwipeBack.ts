...
/**
 * САМОЕ КОРОТКОЕ, ЧЕМ МОЖЕТ ОБОЙТИСЬ СМЕНА (px).
 *
 * Ход таблетки ведёт стык, и по-хорошему он идёт ровно по её ширине: тронулась,
 * когда стык дошёл до левого края, встала — когда прошёл правый. Но у кружка
 * «назад» ширина 38px, и на ней смена читалась бы щелчком, поэтому узкой
 * таблетке начало отодвигаем ВЛЕВО, до этой длины. Конец не двигаем никогда.
 */
const SEAM_SPAN = 150
...
type Stage = {
  ...
  /**
   * Поставить уехавшую страницу на место, но невидимой, — под открытие экрана.
   */
  park(): void
...
function buildStage(under: Snapshot | null): Stage {
  ...
  /**
   * ХОД ТАБЛЕТКИ ИДЁТ ПОД СТЫКОМ, И ТОЛЬКО ПОД НИМ.
   *
   * Раньше отсчёт начинался в четверти таблетки и длился фиксированные 150px,
   * куда бы стык за это время ни дошёл. Для кнопки у левого поля шапки это
   * значило, что смена заканчивалась на 175-м пикселе жеста — а страница едет
   * все 390. Оставшиеся две трети свайпа шапка нижнего экрана просто ЛЕЖАЛА
   * поверх ещё не уехавшего верхнего: «не перетекает, а накладывается».
   *
   * Теперь окно привязано к самой таблетке: 0 — стык у её левого края, 1 —
   * прошёл правый. Что бы ни менялось — корпус пары, гаснущая одиночка,
   * проявляющаяся снизу, — это происходит ровно там, где сейчас граница
   * экранов, и ни пикселем раньше. Узкой таблетке начало отодвигаем влево до
   * SEAM_SPAN: конец обязан остаться на её правом крае.
   */
  const ramp = (left: number, right: number) => {
    const end = Math.max(8, right)
    const start = Math.min(left, end - SEAM_SPAN)
  ...
  const pairMorphs = (fromChipsAll: HTMLElement[], toChipsAll: HTMLElement[]) => {
    ...
    for (const { el: twin, box, rb } of loners) {
      ...
      const q = ramp(rb.left, rb.left + rb.w)
      const fade = (raw: number) => smooth(q(raw))
      morphs.push({
        el: clone,
    ...
    for (let i = 0; i < pairs.length; i++) {
      ...
      // ── Ход СВОЕЙ таблетки ──
      //
      // Не общий ход страницы, а стык, и ровно по МЕСТУ, которое пара занимает:
      // окно — от левого края того из двоих, кто левее, до правого края того,
      // кто правее. Стык дошёл до кнопки — она тронулась; прошёл её насквозь —
      // она уже стала таблеткой нижнего экрана.
      //
      // Так и должно быть: смена происходит под самой границей экранов. Пока
      // ход был фиксированной длины от четверти ширины, кнопка «назад» у левого
      // поля успевала перетечь к 175-му пикселю жеста — и всю оставшуюся дорогу
      // шапка нижнего экрана лежала поверх ещё не уехавшего верхнего.
      const q = ramp(Math.min(ra.left, rb.left), Math.max(ra.left + ra.w, rb.left + rb.w))
      ...
      // ОТПОЧКОВАВШАЯСЯ ПОЛОВИНА РОЖДАЕТСЯ СРАЗУ НА СВОЁМ МЕСТЕ, А НЕ ЕДЕТ К
      // НЕМУ. Пока её корпус шёл по общей коробке, он начинал ход там же, где
      // стоит первая половина, — у левого поля, — и первую треть свайпа лежал
      // ПОВЕРХ соседней таблетки. На «Текстах» это выглядело так: круг лупы
      // (первая пара) накрывался «Фильтрами», которые потом уезжали вправо, —
      ...
      morphs.push({
        ...
        el: ib,
        at: raw => {
          // НАЧИНКА ОТПОЧКОВАВШЕЙСЯ ПОЛОВИНЫ ПРИХОДИТ ВМЕСТЕ СО СВОИМ КОРПУСОМ.
          //
          // Корпус born-половины набирает плотность по быстрому ходу (born,
          // ×2.4), а содержимое шло по обычному — то есть таблетка уже стояла
          // на экране целиком, а воронка в ней ещё лежала под размытием в 7px
          // и «появлялась» под конец жеста. На глаз это ровно то самое:
          // «иконка фильтра пропадает при свайпе, а после свайпа появляется».
  ...
  /**
   * Разморозить страницу: снять заморозку корня, вернуть высоту документа и
   * прокрутку, отпустить слой доков.
   *
   * Отдельно от снятия слоя, потому что порядок разный: на отменённом жесте
   * размораживаем в самом конце, а на состоявшемся — ЗАРАНЕЕ, до монтирования
   * нового экрана (см. park).
   */
  let thawed = false
  const thaw = (fired: boolean) => {
  ...
  return {
    ...
    // ── ЭКРАН ОТКРЫВАЕТСЯ НЕ ЗА КРАЕМ И НЕ В ЗАМОРОЗКЕ ──
    //
    // Переход выполняется под уехавшей страницей, и до сих пор новый экран
    // монтировался ПРЯМО В НЕЙ — в корне, отодвинутом на ширину экрана и
    // примороженном к окну. Отсюда две беды разом.
    //
    // Первая: всё, что компонент меряет на монтировании, возвращало координаты
    // со сдвигом (у урока по ним считается предел ширины докнутого заголовка) —
    // и пересчитывалось заново, когда слой уходил.
    //
    // Вторая: заморозка меняет ПОДЛОЖКУ. Пока она держится, у корня свои
    // `position`, фон и тень; снимаешь их — и у каждой стеклянной таблетки
    // меняется то, из чего она берёт размытие (`backdrop-filter`), а значит и
    // весь её растр. Все таблетки экрана перерисовываются одним кадром: «после
    // свайпа всё будто обновилось и нарисовалось заново».
    //
    // Поэтому размораживаем ЗАРАНЕЕ — до перехода, — а страницу держим
    // невидимой прозрачностью. Именно прозрачностью, а не `visibility`: узел с
    // `opacity:0` браузер всё равно РИСУЕТ, то есть стекло успевает набрать
    // свой растр в окончательной подложке, и показ в конце — только смена
    // альфы. Заодно снимаем пальцы: невидимая страница не должна ловить тапы.
    park() {
      thaw(true)
      movers.forEach(({ el }) => {
        el.style.opacity = '0'
    ...
    destroy(fired) {
      ...
      // z-index — ровно то, что было до жеста. После park’а этим же снимается
      // прозрачность, которой страница держалась невидимой.
      movers.forEach(({ el, css }) => { el.style.cssText = css })
      if (!thawed) thaw(fired)
    },
  }
}
...
function install() {
  ...
  const finish = async (cancelled: boolean) => {
    ...
    if (fired) {
      ...
      returning = true
      // Страница — на место (невидимо), и только потом переход: экран обязан
      // смонтироваться в правильной системе координат.
      localStage?.park()
    ...
    // ── СНИМАЕМ СЛОЙ, КОГДА ЖИВОЙ ЭКРАН УЖЕ НАРИСОВАН ──
    //
    // Ждали фиксированные 80мс. Экран курсов за них не успевает: React ставит
    // разметку, но первый кадр с ней ещё не нарисован, — и слой уходил из-под
    // недорисованной страницы. На глаз это ровно «после свайпа всё обновилось»:
    // шапка появилась, а следом переехала на своё место.
    //
    // Ждём ДВА кадра (первый — коммит React, второй — его растр) и страхуемся
    // таймером: в превью rAF не тикает вовсе (см. память проекта), и без
    // подстраховки страница осталась бы отодвинутой навсегда. Задержка не
    // видна: под слоем всё это время лежит снимок ТОГО ЖЕ экрана.
    let dropped = false
    const drop = () => {
      if (dropped) return
      dropped = true
      ...
      reset()
      // СНИМОК — НЕ СРАЗУ ПОСЛЕ СНЯТИЯ СЛОЯ.
      //
      // Съёмка копирует всё дерево экрана: это десятки миллисекунд основного
      // потока. Пока она стояла в полсекунды, эти миллисекунды приходились
      // ровно на то, чем открывшийся экран занят сам, — монтирование, доводка
      // прокрутки (MobileScreen тянет её тиками по 80мс), первый растр стекла.
      // Стеклянные таблетки в этот момент перерисовываются все разом, и на
      // глаз это читается как «шапка моргнула и обновилась» сразу после свайпа.
      // Ждём, пока экран встанет, и только тогда снимаем.
