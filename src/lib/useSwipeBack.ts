...
function buildStage(under: Snapshot | null): Stage {
  ...
  return {
    ...
    // ── ЭКРАН ОТКРЫВАЕТСЯ НЕ ЗА КРАЕМ, А НА СВОЁМ МЕСТЕ ──
    //
    // Переход выполняется под уехавшей страницей, и до сих пор новый экран
    // монтировался ПРЯМО В НЕЙ — в корне, отодвинутом на всю ширину экрана.
    // Всё, что компонент меряет на монтировании (getBoundingClientRect,
    // наблюдатели пересечения), возвращало координаты, сдвинутые на эту ширину:
    // у урока по ним считается предел ширины докнутого заголовка. Числа
    // получались чужие, а после снятия слоя всё пересчитывалось заново — на
    // экране это и есть «таблетки моргнули на месте сразу после свайпа».
    //
    // Поэтому перед самым переходом страницу возвращаем на место и ГАСИМ
    // видимостью. `visibility:hidden` — не `opacity`, не `display`: вёрстка
    // считается полностью и честно, замеры верные, а на экране по-прежнему
    // снимок. Разморозку делает `destroy` — он и так возвращает cssText целиком.
    park() {
      movers.forEach(({ el }) => {
        el.style.transform = 'none'
        el.style.visibility = 'hidden'
