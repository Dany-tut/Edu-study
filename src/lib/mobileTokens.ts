...
// НИЖЕ неё; 22 px — граница, на которой шапка уже читается целиком.
/**
 * Высота таблетки шапки. ОДНА на все экраны.
 *
 * Свайп «назад» перетекает таблетки соседних экранов друг в друга
 * (lib/useSwipeBack.ts): корпус тянется из кружка в таблетку. Если у экранов
 * высоты разные, на стыке видно, как кнопка дёргается по вертикали, — поэтому
 * высота и вертикальное положение шапки общие, а не «на глаз» у каждого
 * экрана. Раньше в шапках жили 36, 38, 40 и 42.
 */
export const MOBILE_PILL_H = 40

export const MOBILE_TOP_GAP = 22
