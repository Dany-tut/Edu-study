...
const IMAGES: Record<string, string> = {
  ...
  // Слова, которым готовый рисунок подходит целиком.
  //
  // ОДИН РИСУНОК НА НЕСКОЛЬКО ЗНАЧЕНИЙ — это норма карты с самого начала
  // («еда» и «обед» ведут на тот же рис, «поездка» и «путешествие» — на тот же
  // чемодан). Ниже — значения, которые чаще всего просили курсы корейского,
  // английского, немецкого и японского и не получали ничего: рисовать под них
  // отдельный силуэт незачем, «десерт» опознаётся мороженым, а «пешком» —
  // идущей фигурой. Задание «что на картинке» никогда не поставит два таких
  // слова рядом: обманка с той же картинкой отсеивается (см. pictureTasks).
  'супермаркет': shop, 'десерт': iceCream, 'чтение': reading,
  'аудирование': listening, 'пешком': walking, 'время': clock,
  'туризм': suitcase, 'совещание': office, 'выходные': vacation,
}
