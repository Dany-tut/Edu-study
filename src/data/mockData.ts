...
export interface Lesson {
   ...
   * `content` и `homework` (см. fetchLessonsHeavy в lib/db.ts) — они весят в
   * восемь раз больше всего остального и приезжают только к тем урокам,
   * которые ученик открывает. Пока флаг стоит, отсутствие домашки НИЧЕГО не
   * значит: её просто ещё не спросили. Экраны, которым это важно, обязаны
   * различать «нет» и «не знаю» — иначе урок откроется пустым.
   *
   * У большинства уроков флаг не снимется никогда — до них не дойдут. Это не
   * «недогруженное состояние», а норма.
   */
  heavyPending?: boolean
  /**
   * Есть ли у урока сложный уровень — из колонки `lessons.has_hard`
   * (миграция 0067), плюс назначенное из банка.
   *
   * Живёт отдельно от домашки именно потому, что домашка к треку не едет:
   * спутник-звезда должна знать про хард на уроке, который ученик ещё не
   * открывал. См. lessonHasHardLevel.
   */
  hasHard?: boolean
  /**
   * Сложные задания, назначенные группе на этот урок через «Создать домашку».
   * Приезжают со скелетом (запрос лёгкий) и вливаются в `homework`, когда
   * доедет тяжёлая половина, — см. withBankHard в lib/db.ts.
   */
  bankHard?: import('../lib/useHomework').HardTaskDef[]
}
...
export interface Subject {
  ...
  /**
   * uuid курса в БД. `id` — это short_id, которым курс зовётся на клиенте.
   *
   * Заводился под догрузку тяжёлой половины курсами (`lessons.course_id`);
   * сейчас она идёт поурочно, по short_id, и читателей у поля не осталось.
   * Оставлен как честный идентификатор курса — но если так и не понадобится,
   * его стоит убрать, а не тащить.
   */
  dbId?: string
}
