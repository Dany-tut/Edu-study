...
function englishVerdict(results: DiagResults): PlacementVerdict {
  ...
  if (idx === 1) {
    if (b1Strong) return { level: 'B1+', note: 'Средний уровень взят с запасом: условные, пассив и косвенная речь почти без ошибок. Можно переходить к академическому английскому и формату экзамена.', ...IELT, ladder }
  ...
  if (idx === 2) return { level: 'B2', note: 'Уверенный уровень выше среднего: третий тип условных, косвенная речь и академическая лексика на месте. Логичный следующий шаг — целенаправленная подготовка к IELTS.', ...IELT, ladder }
  return { level: 'C1', note: 'Продвинутый уровень: инверсия и сослагательное наклонение не вызывают трудностей. Осталось отточить экзаменационный формат — с такой базой цель IELTS 7.0+ реалистична.', ...IELT, ladder }
}
