...
export const ENGLISH_DESIGN_CAREER: EnglishUnit[] = [
  ...
  },
  {
    ...
    grammarWhy: 'Презентация — это не описание, а проводка. Нужны фразы-указатели, иначе слушатель теряется в экране. Отдельно готовят ответ на вопрос «а что здесь сделал ИИ»: его задают почти всегда, и выигрывает не тот, кто отрицает, а тот, кто называет инструмент и объясняет своё решение.',
    ...
    vocab: [
      ...
      // «Где здесь ИИ, а где вы» — вопрос на защите портфолио в 2026 такой же
      // обычный, как «почему такой шрифт». Отвечают на него РЕШЕНИЕМ, а не
      // оправданием: инструмент назвали, выбор объяснили.
      { term: 'I used AI to explore options', ru: 'я использовал ИИ, чтобы перебрать варианты', alt: ['ИИ для перебора вариантов'] },
      { term: 'the decision was mine', ru: 'решение принимал я', alt: ['выбор за мной', 'решал я сам'] },
  ...
  // ═══ Модуль 4. Оффер и старт ═══
  {
    ...
    grammarWhy: 'Половина проваленных тестовых — это непонятый бриф. Уточняющий вопрос дешевле переделки. Сюда же относится вопрос про ИИ: разрешён он или нет, спрашивают ДО работы, а не после сдачи — иначе выбор инструмента превращается в спор о честности.',
    ...
    vocab: [
      ...
      // Вопрос «а можно с ИИ» задаётся сегодня в каждом втором тестовом, и
      // молчание о нём читается хуже любого ответа: спрашивают не «пользовался
      // ли», а «что ты решил сам».
      { term: 'AI-assisted', ru: 'сделано с помощью ИИ', alt: ['с ИИ', 'с помощью нейросети'] },
      { term: 'Are AI tools allowed?', ru: 'можно ли пользоваться ИИ?', alt: ['разрешён ли ИИ', 'можно ли использовать нейросети'] },
      { term: 'I used it for, not instead of', ru: 'я использовал его для, а не вместо', alt: ['использовал как инструмент, а не вместо себя'] },
