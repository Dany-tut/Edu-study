...
export const PORTUGUESE2_UNITS: LangUnit[] = [
  ...
  },
  {
    ...
    tasks: [
      ...
      // Подчёркивания в поле ответа нет — разметка задаётся тем, что набирается.
      write('Напишите абзац на 10 предложений на любую спорную тему, используя минимум пять разных связок из юнита. После каждой связки в скобках подпишите её функцию: (уступка), (вывод), (пример).'),
    ],
  },
  {
...
export const PORTUGUESE_INTERMEDIATE: LanguageCourseSpec = {
  ...
  units: PORTUGUESE2_UNITS.map(u => ({
    ...
    tasks: [...u.tasks, ...(PORTUGUESE2_EXTRA[u.shortId] ?? [])],
  })),
