...
export interface TaskPayload {
  ...
  altAnswers?: string[]
  /**
   * Скелет ответа — ступень 5 лестницы (docs/MEMORY_STANDARD.md, Р7).
   *
   * ЗАЧЕМ. Между «узнал среди четырёх» (ступень 4) и «написал с нуля»
   * (ступень 6) стоит припоминание С ОПОРОЙ: ученик видит, сколько знаков в
   * слове и с чего оно начинается, — и достаёт из памяти остальное. Без этой
   * ступени курс прыгал с выбора сразу на пустое поле, а пустое поле после
   * четырёх касаний — это ступень 6, поставленная на место пятой.
   *
   * ФОРМА. Первый знак и точки вместо остальных: «안 · · ·». Точки, а не
   * прочерки: «____» голос читает как «андерскор» (см. voiceText в lib/speech).
   *
   * Только у `fill`. Пусто — задание остаётся ступенью 6, как и было.
   */
 ...
 * не туда.
 *
 * ПОЧЕМУ НЕ ХЕШ ОДНОГО ВОПРОСА. Сперва позиция считалась от текста вопроса, и
 * этого мало по двум причинам. Во-первых, у вопроса-близнеца («Что значит
 * 사람?» в лестнице и в экзамене порции) раскладка выходила одна на оба
 * появления: второй раз ученик отвечал по месту, а не по слову. Во-вторых,
 * текст вопроса — не адрес: у генерируемых заданий он повторяется дословно
 * («Что на картинке? Выберите слово.»), и весь класс таких заданий получал
 * одну ячейку на курс.
 *
 * Поэтому солью служит КЛЮЧ ПОЯВЛЕНИЯ (`key`) — адрес задания в курсе, то есть
 * id урока плюс номер задания в нём. Он и уникален, и стабилен между сборками:
 * ровно то, что нужно, чтобы место было непредсказуемым для ученика и
 * неизменным для сохранённых ответов. Ставит его editorTask (languageCourse.ts)
 * — единственное место, где у задания уже есть id.
 *
 * ОБМАНКИ ТОЖЕ ПЕРЕМЕШИВАЮТСЯ, тем же хешем. Раньше двигался только верный
 * вариант, а `[верный, d1, d2, d3]` давал одну и ту же взаимную раскладку
 * обманок во всём курсе: у списка оставался авторский скелет, по которому
 * задание опознаётся, не читая слов.
 */
export function placeCorrect(question: string, choices: string[], correct: number, key = ''): {
  choices: string[]
  correct: number
} {
  const right = choices[correct]
  if (right === undefined || choices.length < 2) return { choices, correct }
  // Числовой ряд и «ни один из перечисленных» перемешивать нельзя (orderMatters).
  if (orderMatters(choices)) return { choices, correct }
  // Разделитель между ключом и вопросом: без него «a» + «bc» и «ab» + «c» —
  // одна и та же строка, то есть одна и та же раскладка у разных заданий.
  // Нулевой знак взят потому, что ни в id задания, ни в тексте вопроса его не
  // бывает: любой печатный разделитель рано или поздно встретится в вопросе.
  // Записан escape-последовательностью, а не сырым байтом: сырой ноль в
  // исходнике делает файл бинарным для grep, file и половины редакторов.
  const h = mixedHash(`${key}\u0000${question}`)
  const n = choices.length
  // mixedHash беззнаковый, поэтому остаток заведомо неотрицательный.
  const at = h % n
  const out = choices
    .filter((_, i) => i !== correct)
    // Номер обманки входит в ключ: два одинаковых по тексту варианта иначе
    // получили бы один вес, и порядок между ними задавала бы сортировка.
    .map((text, i) => ({ text, k: mixedHash(`${h}#${i}#${text}`) }))
    .sort((a, b) => a.k - b.k)
    .map(x => x.text)
...
/**
 * Хеш строки с размешиванием на выходе (FNV-1a + fmix32 из MurmurHash3).
 *
 * ЗАЧЕМ РАЗМЕШИВАНИЕ. У простых полиномиальных хешей (`h = h * 31 + код`)
 * СЛАБЫЕ МЛАДШИЕ БИТЫ: умножение на нечётное последний бит не меняет, поэтому
 * `h % 2` вырождается в чётность суммы кодов строки. Ровно `% 2` нужен паре на
 * слух («что прозвучало — A или B?»), где вопрос у всех заданий дословно один.
 * Три сдвига с умножениями размазывают старшие биты по младшим, и остаток
 * становится ровным при любом делителе.
 *
 * НА НЁМ ЖЕ СИДИТ `placeCorrect`. Пока место считалось от текста вопроса,
 * полиномиального хеша хватало: тексты разнообразны сами по себе. Теперь солью
 * служит АДРЕС задания (`<урок>-l3`), а это строки одного шаблона, отличающиеся
 * парой знаков, — и на вопросах из двух вариантов слабый младший бит сразу дал
 * перекос (курс rulit, место 2 в 46% при случайных 40%). Отдельного хеша у
 * раскладки больше нет.
 */
...
/**
 * «Сборный» вариант: он говорит про ВСЕ остальные, а не стоит с ними в ряд.
 *
 * «Оба варианта возможны», «Ни один не верен», «все варианты» читаются только
 * последними: подняв такой в середину, получаем список, где пункт 2 ссылается
 * на пункты 3 и 4, которых ученик ещё не прочёл.
 *
 * Пробел после «все/всё» обязателен: без него под правило попадали «всегда /
 * обычно / иногда» и «всё взвесив» — обычные варианты, которые двигать нужно.
 */
const CATCH_ALL = /^(вс[её] (перечисленн|вышепереч|вариант|ответ)|об[ае] вариант|ни один|ни одно|никакой|и то,? и другое|нет верн)/i

/** Число в начале варианта: «200–300 знаков» → 200, «2018» → 2018. */
...
/**
 * Список, порядок которого несёт смысл сам по себе, — такой не перемешиваем.
 *
 * Два случая, оба редкие: сборный вариант (см. CATCH_ALL) и числовой ряд по
 * возрастанию или убыванию — «2018 / 2025 / 2030 / 2050», «200–300 / 400–500 /
 * 600–700 знаков». Перемешанный ряд читается заметно хуже, а перевеса не даёт:
 * ученик всё равно не знает, где в нём верный. Заодно правило бережёт шкалы
 * количества («ни одной / одну-две / пять-шесть / чем больше») — они проходят
 * через «ни одной».
 *
 * По всем сидам сюда попадает полтора десятка заданий из двенадцати тысяч.
 * Начнёт срабатывать заметно чаще — значит, регулярка стала ловить обычные
 * варианты, и это повод посмотреть.
 *
 * ДВУХ ВАРИАНТОВ ДЛЯ ЧИСЛОВОГО РЯДА МАЛО: любые два разных числа «упорядочены»
 * в ту или иную сторону, и правило проглотило бы половину лестницы.
 */
export function orderMatters(choices: string[]): boolean {
  if (choices.some(c => CATCH_ALL.test(c.trim()))) return true
  if (choices.length < 3) return false
  const nums = choices.map(leadingNumber)
  if (nums.some(n => n === null) || new Set(nums).size !== nums.length) return false
  const asc = nums.every((v, i) => i === 0 || (v as number) > (nums[i - 1] as number))
  const desc = nums.every((v, i) => i === 0 || (v as number) < (nums[i - 1] as number))
  return asc || desc
}

/**
 * Сторона верного ответа в паре на слух (Р15 — то же правило, другой тип).
 *
 * ЗАЧЕМ. У пары ответ хранится буквой, а не индексом, и мимо `placeCorrect` она
 * проходила незамеченной. Сторону чередовали как `i % 2` — по номеру слова В
 * ПОРЦИИ, с тем же вырождением, что уже вычистили из первого круга лестницы: у
 * порции из одного слова номер всегда нулевой, а таких порций большинство. По
 * девятнадцати сидам «A» выпадала в 62% из 6051 задания.
 */
export function answerSide(key: string): 'A' | 'B' {
  return mixedHash(key) % 2 === 0 ? 'A' : 'B'
}

/**
 * Перемешать варианты задания с НЕСКОЛЬКИМИ верными (Р15).
 *
 * Двигать один ответ здесь нельзя — верных несколько, — поэтому переставляется
 * весь список, а номера верных пересчитываются на новые места. Замер: верные
 * шли первыми в 84 заданиях из 85. Автор пишет сначала то, что подходит, потом
 * обманки, и «отметьте всё верное» решается выбором верхних строк.
 *
 * ПЕРЕСТАНОВКА, А НЕ СДВИГ: сдвиг всего списка сохранил бы верные подряд, а
 * «идут подряд» — половина подсказки.
 */
export function spreadMulti(choices: string[], correct: number[], key: string): {
  choices: string[]
  correct: number[]
} {
  if (choices.length < 2 || correct.length === 0 || orderMatters(choices)) return { choices, correct }
  const order = choices
    .map((_, i) => i)
    .sort((x, y) => mixedHash(`${key}|${choices[x]}`) - mixedHash(`${key}|${choices[y]}`))
  const right = new Set(correct)
  return {
    choices: order.map(i => choices[i]),
    correct: order.map((i, place) => (right.has(i) ? place : -1)).filter(p => p >= 0).sort((a, b) => a - b),
  }
}

/**
 * Порядок ПОКАЗА вариантов: какой исходный номер стоит на каждом месте.
 *
 * ЗАЧЕМ ОТДЕЛЬНО ОТ `placeCorrect`. У курсов-сидов порядок чинится прямо в
 * данных: сид пересобирается, расхождение приезжает учителю «Сверкой». У
 * диагностик и placement-тестов так нельзя — ответ ученика сохраняется как
 * НОМЕР выбранного варианта (diag_results.answers), и перестановка в данных
 * задним числом переврала бы все уже сданные тесты: верный ответ показался бы
 * неверным, а рядом встал бы чужой текст.
 *
 * Поэтому перестановка отдаётся отдельно от данных: показываем в новом порядке,
 * а СОХРАНЯЕМ исходный номер (order[показанный] = исходный). Старые результаты
 * остаются валидными, разбор у учителя работает без единой правки.
 *
 * ПОБОЧНАЯ ПОЛЬЗА. Вопросы, отредактированные учителем, лежат в Supabase и
 * читаются мимо наших массивов — правка данных до них бы не добралась. Через
 * показ их накрывает тем же правилом.
 */
export function displayOrder(options: string[], key: string): number[] {
  const same = options.map((_, i) => i)
  if (options.length < 2 || orderMatters(options)) return same
  return same.sort((x, y) => mixedHash(`${key}|${options[x]}`) - mixedHash(`${key}|${options[y]}`))
}
