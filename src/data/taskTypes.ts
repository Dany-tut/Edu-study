 ...
 *
 * ЗАЧЕМ ОТДЕЛЬНО ОТ `placeCorrect`. Там место считается как `h % количество`
 * от `h = h * 31 + код`, и для четырёх вариантов этого хватает с запасом. Но у
 * простых полиномиальных хешей СЛАБЫЕ МЛАДШИЕ БИТЫ: умножение на нечётное
 * последний бит не меняет, поэтому `h % 2` вырождается в чётность суммы кодов
 * строки. А ровно `% 2` и нужен паре на слух («что прозвучало — A или B?»), и
 * вопрос там у всех заданий дословно один. Три сдвига с умножениями размазывают
 * старшие биты по младшим, и остаток становится ровным при любом делителе.
 *
 * `placeCorrect` намеренно оставлен на своём хеше: он измеряется чисто, а смена
 * места перетасовала бы двенадцать тысяч уже сохранённых заданий — учителя
 * увидели бы это «Сверкой с сидом» на ровном месте.
 */
