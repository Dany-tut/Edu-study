...
/**
 * Список слов вслух — ПО ОДНОМУ СЛОВУ ЗА ЗАДАНИЕ.
 *
 * ПОЧЕМУ НЕ ОДНОЙ ЗАПИСЬЮ. Десять слов в один заход ученик не читает, а
 * отбывает: сбился на третьем — перезаписывай все десять, а ответ «получилось
 * или нет» приходит от учителя через неделю, когда звук уже не в ушах.
 *
 * ПО ОДНОМУ — ЭТО СРАЗУ «ДА» ИЛИ «НЕТ». У каждого слова свой эталон, а значит
 * распознавалка сверяет его тут же (см. VoiceAnswer): сказал — увидел вердикт —
 * при промахе сказал ещё раз, не трогая остальные. Ровно то же слово, ровно тот
 * же набор звуков, но десять секунд вместо сорока пяти и ошибка ценой в одно
 * слово.
 *
 * `hint` — то, что дописывается к слову в формулировке (перевод, напоминание о
 * правиле). Не задан — в вопросе одно слово.
 */
export const readAloudEach = (
  words: string[],
  opts: { prompt?: string; hint?: (w: string) => string; responseSeconds?: number } = {},
): SeedTask[] =>
  words.map(w => readAloud(
    `${opts.prompt ?? 'Прочитайте вслух'}: ${w}${opts.hint ? ` — ${opts.hint(w)}` : ''}`,
    w,
    opts.responseSeconds ?? 15,
  ))
...
/**
 * Задания «что на картинке» из словаря порции.
 *
 ...
 * (§1.9 стандарта — двойное кодирование: картинка помнится лучше слова).
 *
 * ПОЧЕМУ ЗАПАС ОБМАНОК ПРИХОДИТ СНАРУЖИ. Функция была написана под ЮНИТ из
 * десяти слов и брала обманки из него же. Потом появилось правило порции (Р1),
 * и вызывается она теперь с порцией из двух-трёх слов — а в ней ни четырёх
 * слов на проверку длины, ни трёх обманок. Итог: за девятнадцать курсов
 * задание не собралось НИ РАЗУ, при том что функция написана, вызвана и
 * прокомментирована. Поэтому цель берётся из порции (её и учат сегодня), а
 * обманки — из общего запаса уже пройденного (Р9), как в экзамене порции.
 *
 ...
 */
function pictureTasks(unit: LangUnit, pool: VocabItem[], idBase: string, lang: string) {
  const drawn = unit.vocab.filter(w => vocabImage(w.ru))
  if (!drawn.length) return []

  // Одно задание на порцию. Больше — это уже не отработка, а перебор
  // картинок: слов в порции три-четыре, и два экрана подряд «что на картинке»
  // спрашивали бы про соседей по одному и тому же уроку.
  const count = 1
  const tasks = []
  for (let k = 0; k < count; k++) {
    const target = drawn[(unit.n + k) % drawn.length]
    // Обманка не должна иметь ту же картинку, что и ответ: «кот» и «кошка»
    // рисуются одинаково, и такое задание нерешаемо. И не должна повторять
    // сам ответ — в запасе лежит и сегодняшняя порция тоже.
    const targetImage = vocabImage(target.ru)
    const others = pool
      .filter(w => w.term?.trim() && w.term !== target.term && vocabImage(w.ru) !== targetImage)
      .filter((w, i, list) => list.findIndex(o => o.term === w.term) === i)
      .slice(k, k + 3)
...
export interface CourseSummary {
  ...
  vocabCount: number
  /**
   * Отпечаток содержимого сида — по нему видно, что готовый курс ушёл вперёд
   * относительно сохранённой копии учителя.
   *
   * ЗАЧЕМ НЕ СЧИТАТЬ РАЗНИЦУ НА МЕСТЕ. Настоящая сверка (lib/seedSync.ts)
   * требует собрать сид целиком — сотни килобайт на курс. На плитке списка,
   * где таких курсов пятнадцать, это ровно та загрузка, ради избавления от
   * которой карточки и вынесены отдельным файлом.
   *
   * ПОЧЕМУ НЕ ПО СЧЁТЧИКАМ. Число заданий и уроков ловит не всякую правку:
   * сложные задания, например, добавились ВНУТРЬ существующих уроков и ни
   * одного счётчика не сдвинули. Отпечаток считается по содержанию и меняется
   * от любой правки контента.
   *
   * Пишется скриптом `npm run stamp:seeds`; пусто — значит сид ещё не
   * штамповали, и признак на плитке не показывается.
   */
  stamp?: string
  /**
    ...
    parts.forEach((vocab, k) => {
      ...
      const work = breakSameTypeRuns(interleaveCards(
            [
              ...
              ...pictureTasks(part, examPool(seen, introduced), `${id}-pic`, spec.lang),
