...
function parseDelimited(text: string): Card[] {
  ...
  const cards = rows
    ...
    .filter(r => r[0] && r[1])
    .map(r => ({ term: r[0], ru: r[1], note: r[2] || undefined }))
...
[{"term":"<слово или фраза на изучаемом языке>","ru":"<перевод на русский>","note":"<пояснение, если нужно>"}]
...
5. Не выдумывай карточек, которых в источнике нет, и не добирай список до круглого числа.
6. Пропускай служебное: заголовки разделов, номера упражнений, страницы, подписи кнопок,
   меню сайта, рекламу, имена авторов.
7. Порядок — как в источнике.
...
function parseCards(text: string): Card[] {
  ...
  return arr
    ...
    .map(x => {
      ...
      return term ? { term, ru, note: note || undefined } : null
    })
